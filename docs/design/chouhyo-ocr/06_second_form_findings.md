...
- 前提となる机上仮説（2026-09-01・企画担当の実査）: C-1（1帳票=1ページ・`record.pages` は未参照）／C-2（`face_id` は front/back のみ）／C-3（各面に `tables` 1つ以上必須）／C-6（px 定数が 300dpi 較正）。本書はこの4件の裏取り・反証と、実測中に見つかった新規の露呈をあわせて記録する
...
| 2 | `tables` を持たない面はテンプレート読み込み自体が拒否される。メッセージ: `face 'front' に tables が無い。位置合わせのアンカーとして各面に1つ以上のテーブル定義が必要` | `core/chouhyo_ocr/template.py:557-560` | 拒否 | **C-3 と一致**（実測済み・2026-09-01）→ **2026-09-03 解消**（issue #86）。表を持たない面は単発欄の枠線をアンカーに使えるようになり、拒否は「横線・縦線のどちらかが 1 本も作れない面」だけに狭まった。表のある面は経路ごと分かれており挙動は不変（02_design.md D-25） |
...
> **その後（2026-09-03・issue #86）**: C-3 の拒否は緩和して解消した。C-1（`record.pages`）・C-2（`face_id` の 2 値固定）は据え置き——C-2 は編集画面（`Editor.tsx`）が 2 面を決め打ちで生成しており（露呈 6）、コアだけ緩めても作れる様式が増えないため別作業として残す。
...
- C-2/C-3/C-1 はいずれも「拒否」で止まる設計であり、様式追加時にクラッシュ・誤出力が起きるリスクは低い。汎用化の作業は「拒否条件を緩める」設計変更として着手しやすい構造になっている（**C-3 は 2026-09-03 に緩和済み**。残るのは C-2・C-1）
