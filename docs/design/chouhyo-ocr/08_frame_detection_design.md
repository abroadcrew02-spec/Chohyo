...
1. `_same_signature`（垂直レール署名が `COLLINEAR_TOL` 以内で一致）**かつ縦の連続性**（直前行との間隔が直前行の高さの `HEADING_CHAIN_MAX_GAP_RATIO`（3.0）倍以内）で行を y 昇順のチェーンにまとめる（issue #111・2026-09-07 追加）。縦の連続性が無いと、同じ列境界の表がページに複数あるとき（表面・裏面が同じ画像に載る等）全ての行が1本のチェーンに混ざり、本文の等ピッチ判定（手順3）が表と表の間の大きなジャンプで空振りして切り離しが1件も効かなくなる（旧実装の実バグ）。この連続性チェックは、チェーン長がちょうど3（見出し＋本文2行）のとき手順3の `gaps` が1本しか無く等ピッチ検査が恒偽になる退化ケースも副次的に塞ぐ——本文2行目を束ねる時点で直前行（本文1行目）の高さ基準を超える間隔なら、その時点でチェーンに入らない
...
**`stats.heading_detached`（issue #111 (c)・2026-09-07 追加）**: 切り離した行の総数を、run が成立したか（＝提案の `heading_excluded` が付いたか）に関わらず常に返す。`heading_excluded` は run が成立した提案にしか付かないため、切り離した行が run を作れなかった場合（例: 縦の連続性で分かれた別チェーンの本文が短すぎる等）、この件数だけが唯一の手がかりになる。
