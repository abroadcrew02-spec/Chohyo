...
- L1: pytest 1,006 件（collected。GUI スモーク 32 件は dev サーバー無しでは自動 skip・symlink 権限の 2 件も skip）＋ gui-logic 470 件＋ cargo test 154 件＋ tsc（型検査）＋ core-dist 鮮度検査。いずれも `run_all_tests.py` が一括実行する（件数は 2026-09-07 実測・`run_all_tests.py` の集計行）
