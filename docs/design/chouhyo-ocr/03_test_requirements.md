...
| 項目 | 内容 |
|---|---|
| 版 | v1.3（2026-09-04・issue 一括消化ラウンドの反映: §3 の表を実測へ追従・新設 10 スイート・回帰ゲート基準値を 815/341/131/tsc/core-dist へ更新。v1.2=2026-09-01・issue #64 の一括更新: §2/§3 の件数を実測405件へ是正し5巡目〜#66 の新設21スイートを掲載、回帰ゲート基準値の正本を §3 に置いた。v1.1=2026-08-31・v1.0=2026-08-28） |
...
- L1: pytest 831 件（collected。GUI スモーク 14 件は dev サーバー無しでは自動 skip・symlink 権限の 2 件も skip）＋ gui-logic 341 件＋ cargo test 131 件＋ tsc（型検査）＋ core-dist 鮮度検査。いずれも `run_all_tests.py` が一括実行する（件数は 2026-09-04 実測・`run_all_tests.py` の集計行）
