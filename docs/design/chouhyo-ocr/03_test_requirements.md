...
| 項目 | 内容 |
|---|---|
| 版 | v1.5（2026-09-04・表を升の集まりとして扱う（升ごとの出力・升候補と提案・見出し行の切り離し）の反映: 基準値を 915/387/135/tsc/core-dist へ・GUI スモーク 30・`test_output_columns_stage9.py` 34 件・`test_detect_frames.py` 45 件。v1.4=2026-09-04・テンプレート編集の初回読み込み＝候補優先フローの反映: 基準値を 861/355/135/tsc/core-dist へ・GUI スモーク 26・`test_editor_open_config.py` 58 件を追加。v1.3=2026-09-04・issue 一括消化ラウンドの反映: §3 の表を実測へ追従・新設 10 スイート・回帰ゲート基準値を 815/341/131/tsc/core-dist へ更新。v1.2=2026-09-01・issue #64 の一括更新: §2/§3 の件数を実測405件へ是正し5巡目〜#66 の新設21スイートを掲載、回帰ゲート基準値の正本を §3 に置いた。v1.1=2026-08-31・v1.0=2026-08-28） |
...
- L1: pytest 947 件（collected。GUI スモーク 30 件は dev サーバー無しでは自動 skip・symlink 権限の 2 件も skip）＋ gui-logic 387 件＋ cargo test 135 件＋ tsc（型検査）＋ core-dist 鮮度検査。いずれも `run_all_tests.py` が一括実行する（件数は 2026-09-04 実測・`run_all_tests.py` の集計行）
