...
| gui/tests/gui-logic.test.mjs（node 直実行） | 239 | GUI 純関数ロジック（保存前確認の警告合成・列数比較 `columnDecreaseFor`・カウンタ通知 `counterNotice`・出力列タブ・画像なし初期表示の案内 `noImageNotice` と操作可否 `canvasInteractionAllowed` ほか） |
