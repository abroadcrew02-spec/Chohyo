...
| 項目 | 内容 |
|---|---|
| 版 | v1.8（2026-09-08・アプリ全体レビュー（#135〜#166）の改修ラウンドの反映: 基準値を 999/480/167/tsc/core-dist へ・GUI スモーク 33・新設 6 スイート（store_error_run／store_alignment_corruption／editor_cmd_import_resilience／logging_rotation／store_wal_and_chunking／verify_poppler_error_codes）。v1.7=2026-09-07・残 open の消化ラウンド（#103 D-15 の被覆率スケーリング・#63 溢れ診断の可視化・#105 AC-F30 合成素材・#106/#107/#66・#129）の反映: 基準値を 972/470/154/tsc/core-dist へ・GUI スモーク 32・`test_format_mismatch_ratio.py` 6 件・`test_run_overflow_diag.py` 4 件・`test_ac_f30_synthetic_material.py` 2 件を追加。v1.6=2026-09-07・bug-hunt #108〜#128 の改修ラウンドの反映: 基準値を 960/467/154/tsc/core-dist へ・GUI スモーク 32・`test_purge_safe_root.py` 29 件・`test_env_test_mode_gate.py` 8 件・`test_grid_cell_index_dedup.py` 5 件を追加。v1.5=2026-09-04・表を升の集まりとして扱う（升ごとの出力・升候補と提案・見出し行の切り離し）の反映: 基準値を 915/387/135/tsc/core-dist へ・GUI スモーク 30・`test_output_columns_stage9.py` 34 件・`test_detect_frames.py` 45 件。v1.4=2026-09-04・テンプレート編集の初回読み込み＝候補優先フローの反映: 基準値を 861/355/135/tsc/core-dist へ・GUI スモーク 26・`test_editor_open_config.py` 58 件を追加。v1.3=2026-09-04・issue 一括消化ラウンドの反映: §3 の表を実測へ追従・新設 10 スイート・回帰ゲート基準値を 815/341/131/tsc/core-dist へ更新。v1.2=2026-09-01・issue #64 の一括更新: §2/§3 の件数を実測405件へ是正し5巡目〜#66 の新設21スイートを掲載、回帰ゲート基準値の正本を §3 に置いた。v1.1=2026-08-31・v1.0=2026-08-28） |
...
- L1: pytest 1,034 件（collected。GUI スモーク 33 件は dev サーバー無しでは自動 skip・symlink 権限の 2 件も skip）＋ gui-logic 480 件＋ cargo test 167 件＋ tsc（型検査）＋ core-dist 鮮度検査。いずれも `run_all_tests.py` が一括実行する（件数は 2026-09-07 実測・`run_all_tests.py` の集計行）
...
**性能の数値を記録するときの規約（2026-09-07 追加・§6.1 と各 NFR の性能行に適用）**: 基準値として記録してよいのは、他の重い処理を動かしていない状態で測った数値だけ。記録には ①コマンド全文 ②使った素材と反復回数 ③実行日 ④測っている間に並行して動かしていた処理の有無 を必ず併記する。#128 では、この4点が残っていなかったために「遅くなった」のが回帰なのか測定環境なのかの切り分けだけで丸一手かかった。
