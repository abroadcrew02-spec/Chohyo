...
# 0.588・back 0.726・全体 0.652（fallback_rect を含む・2026-09 実測）で
# この値を明確に上回るため、出荷テンプレートでは常に FORMAT_MISMATCH_RATIO
# をそのまま返す＝既存挙動を1バイトも変えない（golden 一致）
_COVERAGE_BASELINE = 0.35
# 面積比が0に近い極端な疎テンプレートでも、明らかに崩れた応答（symbol の
# ほとんどが枠外）まで様式不一致から救わないための上限。issue #103 の合成
# ケースC（印字:記入 1.25 で other/total=0.556）は安全に超える値
# （欄20の合成疎テンプレートで実効閾値 ≈0.693・境界まで余裕 0.137）に置きつつ、
# 1.0 側へ寄せすぎて D-15 そのものを無力化しないよう 0.70 に留める
# （Orchestrator 判断・2026-09-07 レビュー差し戻し。旧 0.85 は単発欄だけの
# 極端な疎テンプレートで D-15 の検出力を落としすぎるとの指摘）
_SPARSE_FORMAT_MISMATCH_CAP = 0.70
