...
def user_templates_dir() -> Path:
    ...
    **本番の呼び出し元は無い**（2026-09-03 時点・#90 L-3）。Rust は
    `match-templates` の `--shipped`／`--candidate` と `--template` に毎回
    テンプレートの絶対パスを渡すため、core がこの関数で保存先を解決する
    場面が実行経路に存在しない（呼び出しはテストのみ——`tests/test_paths.py`・
    `tests/test_match_templates.py`。`cli.py:947`・`cli.py:1042` は同じ規律に
    言及するコメントであって呼び出しではない）。契約としては実装の方が厳しい側
    （core は渡されたパスしか読まない）なので関数は残す——CLI 単体運用と、将来 core 側で解決が要るようになった
    ときに、検証を素通りする実装が新しく書かれるのを防ぐため。
