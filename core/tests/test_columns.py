...
def test_total_is_218(template):
...
def test_segment_boundaries(template):
    ...
    assert (len(person), len(family), len(detail)) == (12, 60, 140)
    # 境界: 管理 | 本人 | 家族 | 明細 の順で連続する
    assert cols[6] == "person_電話番号"        # テンプレート定義順＝物理配置順
    assert cols[17] == "person_備考"
    assert cols[18] == "family_01_続柄"
    assert cols[77] == "family_10_生年月日_日"
    assert cols[78] == "detail_01_来店年月日_年"
    assert cols[217] == "detail_28_品目"
...
def test_validate_rejects_wrong_count(template):
    ...
    with pytest.raises(TemplateError, match="218"):
        validate_v1(smaller)
