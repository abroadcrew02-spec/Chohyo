...
- 累積が 1,000 頁を超えると、GUI の完了画面に「中間データに N ページ蓄積しています（出力の書き出しに M秒）。提出済みのバッチは purge で削除してください。」が出る。N は `run` サマリの `total_done_pages`（`state='done'` の累積件数）、M は `render_seconds`（末尾の書き出しの実測秒）
