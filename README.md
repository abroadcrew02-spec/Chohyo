...
2. アプリを起動し、初回だけ「鍵ファイルを選ぶ」で管理者から渡された JSON 鍵を取り込む（安全な形に変換して保存され、元ファイルは以後不要）
3. 画面の手順どおり: **1** 帳票 PDF のフォルダを選ぶ → **2** Excel の保存先をたしかめる → **3** 読み取りを開始する
4. 完了したら「出力フォルダを開く」→ Excel の「要確認セル数」を大きい順に並べ替え、色つきの 〓 を紙の原本と見比べて直す。合計が 0 になったら完成

途中で閉じても大丈夫（次回は続きから）。インターネット接続が必要。
